...
<head>
    ...
    <title>Company TempReturn #{{ $order->return_code }}</title>
...
<table class="address-table">
    <tr>
        ...
        <td class="address-cell">
            <h4>Seller</h4>
            <strong>{{ $order->parent->warehouse->warehouse_name ?? '' }}</strong><br>
            {{ $order->parent->warehouse->city ?? '' }}<br>
            Phone: {{ $order->parent->warehouse->owner_number ?? '' }}<br>
            TIN: {{ $order->parent->warehouse->tin_no ?? '' }}
        </td>
