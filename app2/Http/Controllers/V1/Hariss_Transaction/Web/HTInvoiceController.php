    ...
    public function exportHtInvoice(Request $request)
{
    ...
    if ($extension === 'pdf') {
        $order = HTInvoiceHeader::with(['warehouse', 'customer'])
            ->where('uuid', $uuid)
            ->firstOrFail();
        $orderDetails = HTInvoiceDetail::with(['item', 'itemuom'])
...
public function filterByExpiry(Request $request)
{
    ...
    $headerIds = HtInvoiceHeader::where('customer_id', $customerId)
        ->pluck('id');

    if ($headerIds->isEmpty()) {
        return response()->json([
            'status' => 'not_found',
            'message' => 'No invoice headers found for this customer.'
        ], 404);
    }
    ...
    $details = HtInvoiceDetail::select(
        ...
        ->where('ht_invoice_detail.uom_id', $upc)
