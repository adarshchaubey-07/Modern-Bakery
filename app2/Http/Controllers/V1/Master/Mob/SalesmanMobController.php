...
protected function generateSalesmanFiles($salesman)
    {
        ...
        $files = [];
        // --------------------------------------------------
        // 1️⃣ GET CUSTOMER IDs (FAST QUERY)
        // --------------------------------------------------
        ...
        $customers = AgentCustomer::select('id', 'route_id', 'warehouse')
            ->whereNull('deleted_at')
            ->where(function ($q) use ($routeId, $warehouseIds) {
                if ($routeId) {
                    $q->where('route_id', $routeId);
                }
                if (!empty($warehouseIds)) {
                    $q->orWhereIn('warehouse', $warehouseIds);
                }
            })
            ->pluck('id')
            ->toArray();
        ...
        // --------------------------------------------------
        if (!empty($customers)) {
            $discounts = Discount::select('id', 'customer_id', 'discount_type', 'discount_value', 'start_date', 'end_date')
            ...
            if ($discounts->isNotEmpty()) {
                $discountJson = $discounts->map(function ($d) {
                    return [
                        'discount_id'   => (string)$d->id,
                        'customer_id'   => (string)$d->customer_id,
                        'discount_type' => $d->discount_type ?? '',
                        'amount'        => $d->discount_value ?? '0',
        ...
        // 3️⃣ PROMOTION FILE
        // --------------------------------------------------
        // $promotions = PromotionHeader::select('id', 'promotion_name', 'from_date', 'to_date', 'warehouse_ids')
        //     ->whereNull('deleted_at')
        //     ->whereIn('warehouse_ids', $warehouseIds)
        //     ->get();
        // if ($promotions->isNotEmpty()) {
        //     $promotionJson = $promotions->map(function ($p) {
        //         return [
        ...
        //     Storage::disk('public')->put($fileName, $promotionJson);
