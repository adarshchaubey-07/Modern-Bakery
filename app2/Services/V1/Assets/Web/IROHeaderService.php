...
class IROHeaderService
{
    ...
    public function getDetailCountWithHeader(array $filters = [])
    {
        try {
            ...
            $headers = IROHeader::with([
                'updatedBy:id,name',
                'warehouse:id,warehouse_code,warehouse_name',
                'details',
                'details.chillerRequest'
            ])
    ...
    public function store(array $data): IROHeader
    {
        ...
        try {
            ...
            if (!isset($data['crf_id'])) {
                throw new \Exception("crf_id is required.");
            }

            $data = array_merge($data, [
                'uuid'     => $data['uuid'] ?? Str::uuid()->toString(),
                'osa_code' => $data['osa_code'] ?? $this->generateOsaCodeDetail(),
            ]);

            $chillerRequest = ChillerRequest::select('warehouse_id')
                ->where('id', $data['crf_id'])
                ->first();

            if (!$chillerRequest) {
                throw new \Exception("Invalid crf_id: {$data['crf_id']}");
            }

            $warehouseId = $chillerRequest->warehouse_id;
            ...
            ChillerRequest::where('id', $data['crf_id'])
                ->update(['status' => 5]);
    ...
    public function findByCrfId($id)
    {
        ...
        $records = IROHeader::where('crf_id', $id)->get();
